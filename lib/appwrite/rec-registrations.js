...
export const sendConfirmationEmail = async (registrationData, year = null, sponsorshipPackageUrl = null) => {
  ...
  const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/rec/send-reg-confirmation-email`, {
    // const response = await fetch("http://localhost:3000/api/rec/send-reg-confirmation-email", {
    method: "POST",
    ...
    >
      ...
      >
        ...
        <tbody>
          ...
          ${formatRecOptionalSessions(data.additionalSessions).length ? `
          <tr style="background-color: #fff;">
            <td style="padding: 8px; border: 1px solid #ddd;"><strong>Additional Sessions</strong></td>
            <td style="padding: 8px; border: 1px solid #ddd;">
              ${formatRecOptionalSessions(data.additionalSessions).join(', ')}
            </td>
          </tr>
          ` : ''}
          <tr style="background-color: #fff;">
