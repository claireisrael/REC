...
export function buildRecBadgeNumber(year, sequence) {
  const yearToken = conferenceYearToken(year) || "0000"
  const seq = Number.parseInt(sequence, 10)
  if (!Number.isFinite(seq) || seq < 1 || seq > 999999) return ""
...
export function formatRecBadgeNumber(value) {
  ...
  const match = normalized.match(/^REC(\d{4})(\d{1,6})$/)
  if (!match) return normalized
  return `REC-${match[1]}-${match[2].padStart(6, "0")}`
}
...
export function parseRecBadgeSequence(badgeNumber, year) {
  ...
  const match = normalized.match(new RegExp(`^REC${yearToken}(\\d{1,6})$`))
...
export function expandRecBadgeNumberCandidates(raw, years = []) {
  ...
  if (/^\d{1,6}$/.test(normalized)) {
    for (const year of years) {
      const built = buildRecBadgeNumber(year, Number.parseInt(normalized, 10))
...
export function getRecScanConfirmationCopy({
  ...
  const conferenceShort = conference.shortName || "REC"
