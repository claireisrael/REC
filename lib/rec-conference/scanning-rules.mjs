...
// Venue names are free text ("Main Entrance" vs "main entrance" vs "Main
// Gate"), so this deliberately matches case-insensitively and tolerates one
// name being a substring of the other. Shared by both the Tera event-list
// filter and the scan-submission check below, so a venue that's shown to an
// operator as scannable can never then be rejected at scan time - the two
// checks disagreeing was itself the bug behind at least one silent
// "scans aren't showing up anywhere" incident.
export function venueMatchesAllowedList(eventVenue, allowedVenues) {
  const venues = (Array.isArray(allowedVenues) ? allowedVenues : [])
    .map(normalizeString)
    .filter(Boolean)
  if (!venues.length) return true

  const venue = normalizeString(eventVenue).toLowerCase()
  if (!venue) return true

  return venues.some((item) => {
    const value = item.toLowerCase()
    return value === venue || value.includes(venue) || venue.includes(value)
  })
}
...
export function eventMatchesOperatorRestrictions(event, operator) {
  ...
  if (!venueMatchesAllowedList(event.venue, operator.allowedVenues)) return false
