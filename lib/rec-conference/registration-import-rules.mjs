...
export function getRegistrationImportSampleRows(templateType, conferenceDays = []) {
  ...
  return REGISTRATION_IMPORT_SAMPLE_ATTENDEES.map((attendee, index) => ({
    ...
    AdditionalSessions: index === 1 ? "Business Forum" : "",
...
export function getCouponImportValidationErrors(coupon, options = {}) {
  ...
  if (Number(coupon.conference) !== Number(options.conferenceYear)) {
    errors.push(`CouponCode is not valid for REC ${options.conferenceYear}.`)
  }
...
function parseOptionalSessions(rawValue, errors) {
  ...
  const values = normalizeRecOptionalSessions(raw.split(/[|;,]/))
  const leftover = raw
    .split(/[|;,]/)
    .map(normalizeLookupValue)
    .filter(Boolean)
    .filter((token) => !["no", "n", "false", "0", "none"].includes(token))

  if (!values.length && leftover.length) {
    errors.push("AdditionalSessions must be blank, Yes, or Business Forum.")
  }
