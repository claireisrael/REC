...
function buildSourceRow(row, templateType) {
  return Object.fromEntries(
    getRegistrationImportHeaders(templateType).map((header) => [
      header,
...
export async function buildRegistrationImportTemplate(templateType, conferenceId = "") {
  ...
  return `\uFEFF${Papa.unparse({
    fields: getRegistrationImportHeaders(templateType),
...
export async function buildRegistrationImportErrorCsv(importId) {
  ...
  const sourceHeaders = getRegistrationImportHeaders(importRecord.templateType)
  const fields = [
    ...sourceHeaders,
    "ImportRow",
    "ImportStatus",
