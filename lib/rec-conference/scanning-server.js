...
function formatBadgeNumber(value) {
  const normalized = normalizeBadgeNumber(value)
  const match = normalized.match(/^REC(\d{4})([A-Z0-9]{6})$/)
  if (match) return `REC-${match[1]}-${match[2]}`
  return normalized
}
...
async function generateUniqueBadgeNumber(conference) {
  const year = String(conference?.year || new Date().getFullYear()).replace(/\D/g, "").slice(0, 4) || "0000"
  for (let attempt = 0; attempt < 20; attempt += 1) {
    const candidate = `REC${year}${randomInt(100000, 1000000)}`
    const existing = await listRestDocuments(config.recBadgeTokensCollectionId, [
      Query.equal("badgeNumber", candidate),
      Query.limit(1),
    ]).catch(() => ({ documents: [] }))
    if (!existing.documents?.length) return candidate
  }
...
async function resolveBadgeToken(qrPayload) {
  ...
  const result = await listRestDocuments(config.recBadgeTokensCollectionId, [
    Query.equal("tokenHash", hashValue(rawToken)),
    Query.equal("isActive", true),
    Query.limit(1),
  ])
  ...
  const badgeNumber = normalizeBadgeNumber(rawToken)
  ...
  return numberResult.documents?.[0] || null
}
