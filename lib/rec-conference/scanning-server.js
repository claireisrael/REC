...
export async function getScannerOwnStats(context, { scanLimit = 25 } = {}) {
  ...
  // These don't depend on each other - fetch together instead of one after
  // the other. Registration/event name lookups below are similarly batched
  // into a single "$id in [...]" query each rather than one request per row,
  // which otherwise turns into dozens of sequential round trips as a
  // scanner's history grows - the dominant source of latency on this
  // endpoint while scanning is actively happening.
  const [scans, conference] = await Promise.all([
    listAll(config.recScansCollectionId, [Query.equal("conferenceId", conferenceId)]),
