...
async function isActiveTeraAllocationSerial(serial) {
  if (!config.recScannerAllocationsCollectionId || !serial) return false
...
export async function getTeraStationContextFromToken(request) {
  ...
  if (!serialNumber || !(await isActiveTeraAllocationSerial(serialNumber))) {
    return null
  }

  await updateRestDocument(config.recScannerSessionsCollectionId, session.$id, { lastSeenAt: nowIso() }).catch(() => {})
  return {
    type: "tera-station",
    session,
    serialNumber,
...
// rejected counts, where they rank among other scanners, and a numbered
// table of their own recent scans. Deliberately returns only the caller's
// own row and own scan history, not every other scanner's identity, so this
// stays "see your own place" rather than a public leaderboard.
export async function getScannerOwnStats(context, { scanLimit = 25 } = {}) {
  assertCollectionConfigured(config.recScansCollectionId, "REC scans")
  const conferenceId = context?.conferenceId
  if (!conferenceId) throw new RecScanningError("Conference is required.", 400, "missing_conference")

  const scans = await listAll(config.recScansCollectionId, [
    Query.equal("conferenceId", conferenceId),
  ])
  ...
  const myScans = scans
    .filter((scan) => scan.scannedBy === context.scannerUserId)
    .sort((left, right) => Date.parse(right.scannedAt || 0) - Date.parse(left.scannedAt || 0))
    .slice(0, safeScanLimit)

  const registrationIds = Array.from(new Set(myScans.map((scan) => scan.registrationId).filter(Boolean)))
  const eventIds = Array.from(new Set(myScans.map((scan) => scan.eventId).filter(Boolean)))
  const [registrations, events] = await Promise.all([
    Promise.all(registrationIds.map((id) => getRestDocument(config.recRegistrationsCollectionId, id).catch(() => null))),
    Promise.all(eventIds.map((id) => getRestDocument(config.recScanEventsCollectionId, id).catch(() => null))),
  ])
...
export async function issueBadgeToken({ conferenceId, registrationId, sendEmail = false }, actorId) {
  ...
  if (existing.documents?.[0]) {
    ...
    rawToken = reusableToken || createBadgeSecret()
    tokenDoc = await updateRestDocument(config.recBadgeTokensCollectionId, tokenDoc.$id, compactDocumentData({
      badgeNumber,
      ...(reusableToken ? {} : { tokenHash: hashValue(rawToken), tokenVersion: Number(tokenDoc.tokenVersion || 1) + 1 }),
      issuedAt: nowIso(),
      issuedBy: actorId || "",
      revokedAt: "",
      revokedBy: "",
...
async function resolveBadgeTokenMatch(qrPayload) {
  ...
  for (const candidate of candidates) {
    const hashed = await listRestDocuments(config.recBadgeTokensCollectionId, [
      Query.equal("tokenHash", hashValue(candidate)),
      Query.equal("isActive", true),
      Query.limit(1),
    ])
    if (hashed.documents?.[0]) return { doc: hashed.documents[0], rawToken: candidate }
  }
  ...
  for (const candidate of candidates) {
    const numberCandidates = expandRecBadgeNumberCandidates(candidate, years)
    for (const badgeNumber of numberCandidates) {
      const numberResult = await listRestDocuments(config.recBadgeTokensCollectionId, [
        Query.equal("badgeNumber", badgeNumber),
        Query.equal("isActive", true),
        Query.limit(1),
      ]).catch(() => ({ documents: [] }))
      if (numberResult.documents?.[0]) return { doc: numberResult.documents[0], rawToken: candidate }
    }
  }
...
export async function listRecentRecScanAlerts({ conferenceId, sinceIso = "", limit = 20 } = {}) {
  ...
  const [scanResult, events] = await Promise.all([
    listRestDocuments(config.recScansCollectionId, [
      Query.equal("conferenceId", conferenceId),
  ...
  return {
    alerts: buildRecScanAlerts(scanResult.documents || [], events),
    since,
    now: nowIso(),
  }
}
