...
// rejected counts and where they rank among other scanners. Deliberately
// returns only the caller's own row plus a rank number, not every other
// scanner's identity, so this stays "see your own place" rather than a public
// leaderboard.
export async function getScannerOwnStats(context) {
  assertCollectionConfigured(config.recScansCollectionId, "REC scans")
